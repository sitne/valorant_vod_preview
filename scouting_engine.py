...
class ScoutingEngine:
        ...
        try:
            while current_time < duration:
                ...
                if progress_callback:
                    progress_callback(current_time / duration, f"Scanning at {current_time:.0f}s")
                ...
                # OCR Batch Detection
                detections = self.ocr_engine.detect_timer_batch([frame], [self.cfg.timer_coords])
                ...
                if detected and det_timer and self.is_timer_in_130s(det_timer):
                    ...
                    if best_matches:
                        best_frame, best_timestamp, best_round, best_score, best_timer = best_matches
    ...
    def run_post_processing(self, detect_agents=False, cluster_formations=False, generate_report=False, report_format="markdown", video_source=None):
        ...
        if generate_report and positions_data:
            self._generate_report(positions_data, clusters, cluster_names, report_format, video_source)
    ...
    def _run_agent_detection(self, analyzer, positions_data):
        ...
        round_files = sorted([
            f for f in os.listdir(self.cfg.output_dir)
            if f.startswith("round_") and f.endswith(".png") and "_full" not in f and "_positions" not in f
        ])
        ...
        for round_file in round_files:
            ...
            if minimap_img is None: continue

            # Load metadata
            metadata_file = os.path.join(self.cfg.output_dir, f"round_{round_num:02d}_metadata.json")
            timestamp = 0.0
            ...
            positions = analyzer.analyze_round(round_num, timestamp, round_file, detections)
    ...
    def _run_clustering(self, positions_data):
        ...
        for cid, rounds in clusters.items():
            cluster_names[cid] = formation_analyzer.name_cluster(cid, rounds, positions_data, team)
            
        formation_analyzer.save_clusters(clusters, cluster_names, positions_data)
        return clusters, cluster_names

    def _generate_report(self, positions_data, clusters, cluster_names, fmt, video_source):
        logger.info("Generating report...")
        generator = ReportGenerator(self.cfg.output_dir)
        content = generator.generate_markdown(positions_data, clusters, cluster_names, video_source)
